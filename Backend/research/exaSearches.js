...
async function callA(company, descriptor = '') {
  ...
  const [companyProfile, competitors, funding] = await Promise.all([
  ...
  console.log(`[Call A] Done — company: ${companyProfile.results.length} results, competitors: ${competitors.results.length}, funding: ${funding.results.length}`);

  return { companyProfile, competitors, funding };
}
...
async function callB(company, descriptor = '') {
  ...
  const [news, founders] = await Promise.all([
  ...
  console.log(`[Call B] Done — news: ${news.results.length} results, founders: ${founders.results.length}`);
